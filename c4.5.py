...
def initializeGlobals():
    ...
    mapAttr[5] = {'aa': 1, 'c': 2, 'e': 3, 'd': 4, 'cc': 5, 'k': 6, 'j': 7, 'm': 8, 'q': 9, 'i': 10, 'r': 11, 'ff': 12, 'x': 13, 'w': 14}
...
def entropy(D):
    ...
    if count == 0 or count == len(D):
        ...
        return (-1 * (count/float(len(D))) * math.log((count/float(len(D))),2)) + (-1 * (1-(count/float(len(D)))) * math.log((1-(count/float(len(D)))),2))
...
def EntropyContGain(att, D):
    ...
    for i in dict_succ:
        ...
        if temp_Succ != 0 and temp_Succ != temp_Count:
            tempGain = (temp_Count/float(count)) * (-1 * (temp_Succ/float(temp_Count)) * math.log((temp_Succ/float(temp_Count)),2)) + (temp_Count/float(count)) * (-1 * ((temp_Count - temp_Succ)/float(temp_Count)) * math.log(((temp_Count - temp_Succ)/float(temp_Count)),2))
...
def EntropyGain(att, D):
    ...
    for i in dict_succ:
        if dict_succ[i] != 0 and dict_succ[i] != dict_count[i]:
            I = I + (dict_count[i]/float(count)) * (-1 * (dict_succ[i]/float(dict_count[i])) * math.log((dict_succ[i]/float(dict_count[i])),2)) + (dict_count[i]/float(count)) * (-1 * ((dict_count[i] - dict_succ[i])/float(dict_count[i])) * math.log(((dict_count[i] - dict_succ[i])/float(dict_count[i])),2))
